...
const w = window,
  ...
  on = (el, events, callback) => {
    if (el)
      events.split(" ").forEach((ev) => el.addEventListener(ev, callback));
  },
...
export default class AG extends EE {
  ...
  static set defaultOptions(opts) {
    Object.keys(opts)
      .filter((key) => key in AG.#defaultOptions)
      .forEach((key) => (AG.#defaultOptions[key] = opts[key]));
  }
  ...
  static #build(options) {
    ...
    on(overlay, "swipe cancel", (e) => {
      ...
      if (e.detail.orientation == "hor") {
        ...
        Math.abs(e.detail.delta.y) > 0.2 * dE.clientHeight
      ) {
        AG.#hide();
      }
    });
    ...
    on(overlay, "mousemove", (e) => {
      ...
      if (PD.type == "mouse") {
        ...
        if (item.caption) {
          const itemsLength = item.container.length,
            container =
              itemsLength == 1 ||
              (item.container.length > 1 && item.id < itemsLength - 1)
                ? item.container[0]
                : item.container[1],
  ...
  static #buildSlider() {
    ...
    function buildItem(item, id) {
      const //Add caption if item.caption is defined
     ...
     */
    items.forEach(
      (i) => (i.container = $$(overlay, `figure[data-${g}-id="${i.id}"]`))
    );
  }
  //End #buildSlider
  ...
  static #show() {
    ...
    AG.#emitter.emit(`${g}-opening`, {
      current: currentGallery.currentItem,
      gallery: currentGallery,
    });
    currentGallery.emit(`${g}-opening`, {
      current: currentGallery.currentItem,
    });
    ...
    on(overlay, "transitionend", function waitForTransitionEnd(e) {
      if (e.propertyName == "opacity" && e.target == overlay) {
        ...
        AG.#emitter.emit(`${g}-open`, {
          current: currentGallery.currentItem,
          gallery: currentGallery,
        });
        currentGallery.emit(`${g}-open`, {
          current: currentGallery.currentItem,
        });
      }
    });
  ...
  static #hide() {
    ...
    const currentGallery = AG.#currentGallery,
      overlay = AG.#overlay;

    //Emit amst_gallery-closing event
    AG.#emitter.emit(`${g}-closing`, {
      current: currentGallery.currentItem,
      gallery: currentGallery,
    });
    currentGallery.emit(`${g}-closing`, {
      current: currentGallery.currentItem,
    });
    ...
    on(overlay, "transitionend", function waitForTransitionEnd(e) {
      if (e.propertyName == "opacity" && e.target == overlay) {
        ...
        AG.#emitter.emit(`${g}-close`, {
          current: currentGallery.currentItem,
          gallery: currentGallery,
        });
        currentGallery.emit(`${g}-close`, {
          current: currentGallery.currentItem,
        });
      }
    });
    ...
    new Array(`${g}-show`, `${g}-no-arrow`, `${g}-hide-controls`).forEach((c) =>
  ...
  static #keyboardListener(e) {
    ...
    let keys = ["Escape", "Esc"];
    if (AG.#currentGallery.#items.length > 1)
      keys.push("ArrowLeft", "Left", "ArrowRight", "Right");
  ...
  static #resize() {
    ...
    requestAnimationFrame((_) => {
      const windowWidth = dE.clientWidth,
      ...
      $$(AG.#overlay, "figure").forEach((figure) => {
        ...
        if (item.ratio < windowRatio) {
          ...
          if (caption) {
            ...
            let css = `width:${Math.ceil(
              Math.min(windowHeight * item.ratio, item.width)
            )}px;`;
          ...
          if (caption) {
            const imgHeight = Math.min(windowWidth / item.ratio, item.height),
  ...
  #load(id) {
    ...
    if (
      ...
      $(AG.#overlay, `figure[data-${g}-id="${id}"] img`)
    )
      return;
    ...
    function buildItemContent(i) {
      ...
      content +=
        i.width > 0 && i.height > 0
          ? ` style="max-width:${i.width}px;max-height:${i.height}px; aspect-ratio:${i.ratio};"`
          : ``;
      content += "/>";
      return content;
    }
    ...
    on($(item.container[0], "img"), "load", function loaded() {
        ...
        function check(w, h) {
          ...
          if (shouldUpdateWidthHeight) {
            ...
            item.container.forEach((figure) => {
              ...
              i.setAttribute(
                "style",
                `max-width:${w}px; max-height:${h}px; aspect-ratio:${item.ratio};`
              );
            });
      ...
      if (
        AG.#visible &&
        remainToLoad.length > 0 &&
        self.#items.filter((i) => i.loading).length == 0 &&
        item.previous != -1 &&
        AG.#currentGallery == self
      ) {
        if (remainToLoad == 1) {
  ...
  #goTo(id, transition = false, direction = "") {
    ...
    if (this.#items.length > 1) {
      ...
      */
      ...
      const slider = $(AG.#overlay, `.${g}-slider`),
      ...
      if (this.#currentId == 0 && direction == "prev") {
        ...
        slider.style.transform = `translateX(${
          (-100 / numberOfContainers) * (this.#items.length + 1)
        }%)`;
      } else {
  ...
  #updateOptions() {
    ...
    function camelCase(p, camel = false) {
      return p
        .split("-")
        .map((w, id) =>
          id == 0 && camel ? w : w.charAt(0).toUpperCase() + w.slice(1)
        )
        .join("");
    }
    if (!AG.#overlay) {
      ...
      const navs = $$(AG.#overlay, `.${g}-nav-item`),
  ...
  constructor(selector, options = {}, callback) {
    ...
    const elements =
        typeof selector === "string"
          ? Array.from($$(d, selector))
          : Array.from(selector),
      links = elements.filter(
        (el) =>
          (el.tagName == "A" && el.href.match(AG.#regexp)) ||
          (el.hasAttribute("data-href") &&
            el.getAttribute("data-href").match(AG.#regexp))
      ),
    ...
    links.forEach((link, id) => {
      ...
      o.alt =
        link.getAttribute("data-alt") ||
        ($(link, "img") && $(link, "img").getAttribute("alt")) ||
        "";
